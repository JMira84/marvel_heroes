...
function getHero() {
    ...
    if (HERO_SEARCH.value === "") {
        ...
        ALERT_MSG.textContent = 'Please write the name of a Marvel character';
    } else {
    ...
    Http.fetchData(URL)
        .then(responseData => {
            const HERO_DATA = new HeroData(HERO, responseData.data.results[0].description, responseData.data.results[0].thumbnail.path, responseData.data.results[0].comics.available);
